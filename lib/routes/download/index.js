...
async function download(server, options, done) {
  server.route({
    ...
    handler: async (request, response) => {
      ...
      const { rowCount, rows: batchRows } = await server.pg.read.query(selectBatch({ batchTag, date }))

      if (rowCount === 0) {
        response.status(204)
      } else {
        const [{ id: batchId, tag: newBatchTag }] = batchRows
        const { rows: exposureRows } = await server.pg.read.query(selectExposures({ batchId, id }))
